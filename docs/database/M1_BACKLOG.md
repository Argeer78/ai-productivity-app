...
1. Reconstruct one canonical server-side admin authorization model; remove `NEXT_PUBLIC_ADMIN_EMAIL` authority and reconcile email-only `admin_users` with `profiles.is_admin`.
