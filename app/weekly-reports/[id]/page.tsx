// app/weekly-reports/[id]/page.tsx
"use client";
...
export default function WeeklyReportDetailPage({ params }: PageProps) {
  // ✅ Notes-style + correct namespace: weeklyHistory.*
  const { t: rawT } = useT("");
  ...
  const planLabelUpper = t(`plan.${plan}`, plan === "founder" ? "FOUNDER" : plan.toUpperCase());
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const userPlan: PlanType = rawPlan === "pro" || rawPlan === "founder" ? rawPlan : "free";
        setPlan(userPlan);
        ...
        if (reportError && reportError.code !== "PGRST116") {
          ...
          setError(t("loadError", "Failed to load weekly report."));
          setReport(null);
          return;
        }

        if (!reportRow) {
          setError(t("notFoundError", "Weekly report not found."));
          setReport(null);
          return;
        }
        ...
        setActionPlan(planRow ? (planRow as WeeklyActionPlan) : null);
      } catch (err) {
        console.error(err);
        setError(t("loadError", "Failed to load weekly report."));
        setReport(null);
  ...
  async function handleGenerateActionPlan() {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        ...
        setPlanError(data?.error || t("planGenerateError", "Could not generate weekly action plan. Please try again."));
        return;
      }
      ...
      setPlanSuccess(t("planGenerateSuccess", "Weekly action plan generated."));
    } catch (err) {
      console.error(err);
      setPlanError(t("planNetworkError", "Network error while generating weekly action plan."));
    } finally {
    ...
    return (
      <main className="min-h-screen bg-[var(--bg-body)] text-[var(--text-main)] flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4">
          <h1 className="text-2xl font-bold mb-3">{t("viewer.title", "Weekly Productivity Report")}</h1>
    ...
    return (
      <main className="min-h-screen bg-[var(--bg-body)] text-[var(--text-main)] flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <p className="text-[var(--text-muted)] text-sm">{t("loading", "Loading weekly reports…")}</p>
        </div>
      </main>
    );
  }
      ...
      <div className="flex-1">
        <div className="max-w-3xl mx-auto px-4 py-8 md:py-10 text-sm">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold mb-1">{t("viewer.title", "Weekly Productivity Report")}</h1>
          ...
          <div className="mb-4">
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[var(--border-subtle)] bg-[var(--bg-card)] text-[11px] text-[var(--text-muted)]">
              <span className={`h-2 w-2 rounded-full ${isPro ? "bg-emerald-400" : "bg-amber-400"}`} />
              {t("planLabel", "Plan:")}{" "}
              <span className="font-semibold uppercase text-[var(--text-main)]">{planLabelUpper}</span>
            </span>
          </div>
          ...
          <section className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-4 mb-6">
            <p className="text-xs font-semibold text-[var(--text-muted)] mb-2">
              {t("viewer.summaryLabel", "WEEKLY SUMMARY")}
            </p>
            <div className="text-[12px] text-[var(--text-main)] whitespace-pre-wrap">
              {report.summary || t("viewer.noSummary", "This weekly report has no summary text.")}
            </div>
          </section>

          {/* Action plan section left as-is but under weeklyHistory.* (fallbacks if missing) */}
          <section className="rounded-2xl border border-[var(--accent)]/40 bg-[var(--accent-soft)]/70 p-4 mb-6">
            <p className="text-xs font-semibold text-[var(--accent-strong,_var(--accent))] mb-2">
              {t("viewer.actionPlanTitle", "WEEKLY ACTION PLAN (AI)")}
            </p>

            {planError && <p className="text-[11px] text-red-400 mb-2">{planError}</p>}
            {planSuccess && <p className="text-[11px] text-emerald-400 mb-2">{planSuccess}</p>}

            {!isPro ? (
              <>
                <p className="text-[12px] text-[var(--text-main)] mb-2">
                  {t("viewer.actionPlanProOnly", "AI-powered weekly action plans are a Pro feature.")}
                </p>
                <Link
                ...
                {actionPlan ? (
                  <div className="mb-3">
                    <p className="text-[11px] text-[var(--text-muted)] mb-1">
                      {t("viewer.savedPlanLabel", "Your saved action plan for this week:")}
                    </p>
                    <div className="text-[12px] text-[var(--text-main)] whitespace-pre-wrap">{actionPlan.plan_text}</div>
                  </div>
                ) : (
                  <p className="text-[12px] text-[var(--text-main)] mb-3">
                    {t("viewer.generatePlanHint", "Generate a focused action plan for this week based on your report.")}
                  </p>
                )}
